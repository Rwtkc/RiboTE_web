...
function buildInitialState(defaults) {
  return {
    ...
    dataScope: defaults.data_scope ?? "displayed",
...
export default function EnrichmentExport({ config }) {
    ...
    <Stack spacing={2}>
      ...
      <div className="ribote-control-section ribote-control-section--export ribote-control-section--export-data-plain">
        ...
        <SelectField
          ...
          onChange={(event) => setState((current) => ({ ...current, dataScope: event.target.value }))}
          options={choices.dataScope || []}
        />
        <SelectField
          id={ids.dataFormat}
          label="Data Format"
  ...
  ), [choices.dataFormat, choices.dataScope, choices.format, dataDisabled, figureDisabled, ids, state]);
  ...
  return (
    <ThemeProvider theme={riboteSelectTheme}>
      <Accordion defaultExpanded disableGutters elevation={0} sx={cardSx}>
        ...
        <AccordionDetails sx={accordionDetailsSx}>{content}</AccordionDetails>
      </Accordion>
    </ThemeProvider>
  );
}
