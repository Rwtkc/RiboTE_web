...
export default function ClusteringExport({ config }) {
  if (!config?.ready) {
    return null;
  }
  ...
  useEffect(() => {
    syncInputValue(ids.scope, state.scope);
    ...
    <Stack spacing={2}>
      <div className="ribote-control-section ribote-control-section--export ribote-control-section--export-scope-plain">
        ...
        <SelectField
          ...
          label="Scope"
          value={state.scope}
          onChange={(event) => setState((current) => ({ ...current, scope: event.target.value }))}
          options={choices.scope || []}
        />
      </div>
  ...
  ), [choices.dataFormat, choices.format, choices.scope, dataDisabled, figureDisabled, ids, state]);
